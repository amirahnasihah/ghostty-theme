...
const float MAX_RADIUS = 0.065;
const float RING_THICKNESS = 0.013;
const float CURSOR_WIDTH_CHANGE_THRESHOLD = 0.5;
vec4 COLOR = iCurrentCursorColor;  // matches your #007972 cursor
...
float easeOutBounce(float t) {
    ...
    else if (t < 2.0 / d1) return n1 * (t -= 1.5 / d1) * t + 0.75;
...
float exponentialDecayPulse(float t) { return exp(-3.0 * t) * sin(t * 3.1415916); }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    if (isModeChange > 0.0 && isAnimating > 0.0) {
        ...
        // soft linear fade so the ring dissolves as it expands
